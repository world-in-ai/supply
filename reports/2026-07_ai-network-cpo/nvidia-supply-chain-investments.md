...
대형 전략 투자(A) 중심으로 정리. 금액은 언론 보도 기준.

| 주요 업체 | 유형 | 투자 내용 | 투자 금액 (시점) |
|-----------|------|-----------|------------------|
| **Coherent** | 광 부품·레이저·CPO 광엔진 (전략 파트너십) | 지분 + 다년 수십억 달러 구매약정 + 캐파/우선접근권, 美 생산라인 증설 지원 | **약 20억 달러** (2026.03) |
| **Lumentum** | CW 레이저(광원) (전략 파트너십) | 지분 + 구매약정 + 캐파 확보권, 美 신규 팹 지원 | **약 20억 달러** (2026.03) |
| **Marvell** | 커스텀 XPU·광 DSP·실리콘 포토닉스 (지분) | NVLink Fusion 연계, Celestial AI(Photonic Fabric) 기반 | **약 20억 달러** (2026.03~04) |
| **Corning** | 광섬유·광 커넥티비티 (광범위 딜) | 美 광섬유 캐파 50%+·커넥티비티 10배 확대, 공장 3곳 | **약 32억 달러** (광범위 딜 기준, 2026.05) |
| — **합계** — | — | 위 4건 합산 | **약 92억 달러** |

> 금액 주의:
> - 대형 4건은 지분+구매약정+캐파권이 섞인 패키지 딜의 **총 규모**(순수 지분과 다름). Corning은 "5억 달러 주식취득권" 보도와 별개로 **광범위 딜 규모 32억 달러** 기준으로 표기.
> - **스타트업(Ayar Labs·Xscape 등)은 표에서 제외** — 엔비디아가 **참여한 라운드 전체 규모**만 알려졌고 **엔비디아 단독 출자액은 비공개**이기 때문. (상세는 아래 B섹션 참고.) Lightmatter 등은 CPO 경쟁사이나 엔비디아 투자가 확인되지 않아 제외.
> - 합계 **92억 달러**는 위 4건의 딜 규모를 단순 합산한 값. 언론이 보도한 엔비디아 광통신 **커밋 총액은 "65억 달러+"**로, Corning을 딜 규모로 잡은 본 표와 산정 기준이 다름.
...
| **Corning** | 약 5억 달러 (광범위 딜은 최대 ~32억 달러) | 주식 취득권(rights) + 캐파 파트너십 | 2026.05.06 | **광섬유·광 커넥티비티**. Corning은 美 광섬유 생산 캐파 **50%+ 확대**, 광 커넥티비티 캐파 **10배 확대**, NC·TX에 **신규 공장 3곳** 건설 |

### 딜별 상세

- **Coherent·Lumentum** — 원래 GTC 2025 생태계 협력사였는데, **2026년 3월 2일 각각 약 20억 달러 전략 파트너십**(합계 ~40억 달러)으로 격상. 단순 공급계약이 아니라 **지분 + 다년 수십억 달러 구매약정 + 미래 캐파/우선접근권**이 결합된 구조 → 사실상 두 회사의 광 부품·레이저 생산라인을 엔비디아 쪽으로 **예약(선점)**. 자금은 **미국 내 생산능력 증설**(Lumentum 신규 팹 등)에 투입. **비독점** 조건이라 두 회사는 타 고객도 계속 공급 가능.
  - Coherent·Lumentum은 **CPO의 핵심 병목인 레이저(광원)** 공급사 → 이 투자는 차세대 AI 인프라의 **핵심 공급망을 직접 확보**하는 의미.
- **Marvell** — **약 20억 달러 지분 투자**. Marvell의 **커스텀 XPU 실리콘 + 광 DSP**를 **NVLink Fusion**을 통해 NVIDIA 생태계에 결속(파트너십을 생태계 락인으로 전환). 기반 기술은 2025.12 발표·**2026.02 종결**된 **Celestial AI 인수**(32억 5,000만 달러 = 현금 10억 달러 + 주식 22억 5,000만 달러, 언아웃 포함 최대 55억 달러)로 확보한 **Photonic Fabric** — GPU가 외부 메모리 풀에 근거리급 속도로 접속하게 하는 광 인터커넥트.
- **Corning** — **약 5억 달러 주식 취득권**(광범위 파트너십 기준 보도상 최대 ~32억 달러 규모). AI 데이터센터용 **광섬유·광 커넥티비티** 공급 확대가 핵심. Corning은 美 **광섬유 생산 캐파 50%+**, **광 커넥티비티 캐파 10배** 확대와 **NC·텍사스 신규 공장 3곳** 건설을 약속 → 광섬유 물량을 엔비디아 수요에 맞춰 선제 증설.

## B. 스타트업 벤처 투자 (NVentures / 전략 라운드 참여)

| 대상 | 라운드 | 엔비디아 역할 | 기술 |
|------|--------|---------------|------|
| **Ayar Labs** | Series E, 5억 달러 (2026.03, 밸류 37억 5,000만 달러) | 라운드 참여 | 인패키지 **광 I/O(optical I/O)**, CPO 선구자 |
| **Xscape Photonics** | Series A 4,400만 달러 (+후속 3,700만 달러) | 참여(기존 투자자) | 다파장(8-color) 레이저 **FalconX** |

- **Ayar Labs**: CPO/광 I/O의 대표 스타트업. 엔비디아는 **2023.05 Series C(1억 5,500만 달러)부터 참여**해온 장기 투자자이며, 2026.03 **Series E(5억 달러, 밸류 37억 5,000만 달러, 누적 8억 7,000만 달러)**를 AMD·Intel Capital·Neuberger Berman 등과 함께 참여. 가속기 패키지 안에 광 인터페이스를 직접 넣는 "in-package optical I/O"로, 스위치를 넘어 **가속기 단(scale-up)까지 광 확장**을 겨냥.
...
> 주의: 투자 금액은 언론 보도 기준의 근사치이며, 일부는 지분·구매약정·캐파 확보권이 혼합된 패키지 딜이라 "순수 지분 규모"와 다를 수 있음. Corning은 보도에 따라 "5억 달러 주식취득권"과 "최대 32억 달러 광범위 딜"이 함께 언급됨.
